...
const ROWS = {
  ...
  // hero
  'hero.title': ['把一段长视频，<br /><em>变成可以追溯的笔记。</em>', 'Turn a long video<br /><em>into notes you can trace back.</em>', 'Gör en lång video<br /><em>till anteckningar du kan spåra.</em>'],
  ...
  'upload.url.desc': ['支持 B 站视频页或 b23.tv 短链（最高 720P），以及视频文件直链（如 .mp4）。YouTube、抖音需要登录验证，会拦截服务器下载，请先保存到本地再上传。',
    'Works with Bilibili video pages or b23.tv short links (up to 720p) and direct links to video files (e.g. .mp4). YouTube and Douyin require a signed-in browser and block server downloads, so save those videos first and upload the file.',
    'Fungerar med Bilibili-videosidor eller b23.tv-kortlänkar (upp till 720p) och direktlänkar till videofiler (t.ex. .mp4). YouTube och Douyin kräver inloggning och blockerar nedladdning från servrar, så spara videon först och ladda upp filen.'],
  'upload.url.aria': ['视频链接', 'Video link', 'Videolänk'],
  'limits.kicker': ['// limits', '// limits', '// gränser'],
  'limits.title': ['使用须知 · 实测边界', 'What works well · measured limits', 'Vad fungerar · uppmätta gränser'],
  'limits.hint': ['演示服务器为 2 核 2 GB，以下均为线上实测', 'Demo server: 2 cores, 2 GB. Figures measured live', 'Demoservern har 2 kärnor och 2 GB. Siffrorna är uppmätta'],
  'limits.good': ['效果好', 'Works well', 'Fungerar bra'],
  'limits.good.1': ['1 小时以内、以讲解为主的课程录像（59 分钟实测 4.3 分钟出结果）', 'Lecture recordings up to an hour, mostly speech (59 min measured: result in 4.3 min)', 'Föreläsningar upp till en timme, mest tal (59 min uppmätt: resultat på 4,3 min)'],
  'limits.good.2': ['本地上传（mp4 / mov / mkv / webm）或 B 站视频页、b23.tv 短链、视频文件直链', 'Local upload (mp4 / mov / mkv / webm), Bilibili pages, b23.tv short links or direct video-file links', 'Lokal uppladdning (mp4 / mov / mkv / webm), Bilibili-sidor, b23.tv-länkar eller direktlänkar till videofiler'],
  'limits.good.3': ['同一个视频换目标再问，会复用预处理结果，约 40 秒出结果', 'Asking the same video a new question reuses its preprocessing: about 40 s', 'En ny fråga om samma video återanvänder förbehandlingen: cirka 40 s'],
  'limits.slow': ['会很慢', 'Slow', 'Långsamt'],
  'limits.slow.1': ['超过 1 小时的视频：预处理加分析会超过 5 分钟，建议先剪成几段', 'Videos over an hour: preprocessing plus analysis exceeds 5 minutes; cut them into parts first', 'Videor över en timme: förbehandling och analys tar över 5 minuter; dela upp dem först'],
  'limits.slow.2': ['画面文字识别每个视频最多取 40 张截图：长视频里的板书、幻灯片可能漏掉一部分，但语音内容完整', 'On-screen text is read from at most 40 frames per video: some slides or board writing in long videos may be missed, speech is always complete', 'Text i bild läses från högst 40 bilder per video: vissa bilder i långa videor kan missas, talet är alltid komplett'],
  'limits.slow.3': ['B 站链接偶尔会遇到 CDN 不稳定：一般 1–2 分钟导入完成（1 小时视频约 100 秒），失败时稍后重试或改为本地上传', 'Bilibili links occasionally hit an unstable CDN: imports usually take 1–2 minutes (about 100 s for a 1-hour video); if one fails, retry later or upload the file', 'Bilibili-länkar träffar ibland ett instabilt CDN: importen tar oftast 1–2 minuter (cirka 100 s för en timmes video); försök igen senare eller ladda upp filen'],
  'limits.no': ['不支持', 'Not supported', 'Stöds inte'],
  'limits.no.1': ['YouTube、抖音、Vimeo 链接：平台要求登录或浏览器验证，会拦截服务器下载，请先保存到本地再上传', 'YouTube, Douyin and Vimeo links: they require a signed-in browser and block server downloads; save the video and upload it', 'YouTube-, Douyin- och Vimeo-länkar: de kräver inloggning och blockerar servrar; spara videon och ladda upp den'],
  'limits.no.2': ['B 站会员专享、付费、地区限制的视频，以及番剧、直播（没有 BV 号）', 'Bilibili members-only, paid or region-locked videos, anime episodes and live streams (no BV id)', 'Bilibili-videor för medlemmar, betalda eller regionlåsta, samt anime och livesändningar (inget BV-id)'],
  'limits.no.3': ['纯音乐、无人声也无文字的视频：没有可分析的内容，结果只会描述空白', 'Music-only videos without speech or text: there is nothing to analyse', 'Videor med bara musik, utan tal eller text: det finns inget att analysera'],
  'limits.measured': ['线上实测（每次分析的模型费用）', 'Measured live (model cost per analysis)', 'Uppmätt live (modellkostnad per analys)'],
  'limits.col.video': ['视频', 'Video', 'Video'],
  'limits.col.time': ['分析耗时', 'Analysis time', 'Analystid'],
  'limits.col.cost': ['费用', 'Cost', 'Kostnad'],
  'limits.col.note': ['说明', 'Note', 'Kommentar'],
  'limits.row1.v': ['9.9 分钟 · 数学动画（画面切换多）', '9.9 min · maths animation (many cuts)', '9,9 min · matteanimation (många klipp)'],
  'limits.row1.t': ['86 秒', '86 s', '86 s'],
  'limits.row1.c': ['¥0.10', '¥0.10', '¥0,10'],
  'limits.row1.n': ['优化前超过 5 分钟', 'Over 5 min before the optimisation', 'Över 5 min före optimeringen'],
  'limits.row2.v': ['59 分钟 · 课程录像（本地上传）', '59 min · lecture recording (uploaded)', '59 min · föreläsning (uppladdad)'],
  'limits.row2.t': ['4.3 分钟', '4.3 min', '4,3 min'],
  'limits.row2.c': ['¥0.18', '¥0.18', '¥0,18'],
  'limits.row2.n': ['接近 5 分钟上限；上传 414 MB 约 1.5 分钟', 'Close to the 5-minute limit; uploading 414 MB took 1.5 min', 'Nära gränsen på 5 minuter; uppladdning av 414 MB tog 1,5 min'],
  'limits.row3.v': ['同一视频 · 换一个问题', 'Same video · new question', 'Samma video · ny fråga'],
  'limits.row3.t': ['约 40 秒', '≈ 40 s', '≈ 40 s'],
  'limits.row3.c': ['¥0.08', '¥0.08', '¥0,08'],
  'limits.row3.n': ['复用预处理结果', 'Preprocessing reused', 'Förbehandlingen återanvänds'],
  'limits.row4.v': ['3.2 分钟 · 纯钢琴曲', '3.2 min · solo piano', '3,2 min · solopiano'],
  'limits.row4.t': ['约 1.5 分钟', '≈ 1.5 min', '≈ 1,5 min'],
  'limits.row4.c': ['¥0.05', '¥0.05', '¥0,05'],
  'limits.row4.n': ['能跑完，但没有可分析的内容', 'Finishes, but there is nothing to analyse', 'Blir klar, men inget att analysera'],
  'limits.row5.v': ['1 小时视频 · 链接导入', '1-hour video · by link', 'Entimmesvideo · via länk'],
  'limits.row5.t': ['下载约 100 秒', 'Download ≈ 100 s', 'Nedladdning ≈ 100 s'],
  'limits.row5.c': ['—', '—', '—'],
  'limits.row5.n': ['分块并行下载，原来要 30 分钟以上', 'Parallel chunked download; it used to take over 30 minutes', 'Parallell nedladdning i delar; tog tidigare över 30 minuter'],
  'limits.todo': ['优化记录与后续方向', 'Done and next', 'Klart och nästa steg'],
  'limits.todo.1': ['已完成：语音转写从逐段串行改为 10 路并行（10 分钟视频 104 秒 → 26 秒）', 'Done: transcription runs 10 segments in parallel instead of one by one (10-min video: 104 s → 26 s)', 'Klart: transkribering av 10 segment parallellt i stället för ett i taget (10 min video: 104 s → 26 s)'],
  'limits.todo.2': ['已完成：截图只解码关键帧、缩到 1280 px，数量按时长封顶 40 张（10 分钟视频 265 秒 → 42 秒）', 'Done: frames come from key frames only, scaled to 1280 px, at most 40 per video (10-min video: 265 s → 42 s)', 'Klart: bilder bara från nyckelbilder, skalade till 1280 px, högst 40 per video (10 min video: 265 s → 42 s)'],
  'limits.todo.3': ['边转写边分析：先返回前几分钟的结论', 'Stream results: answer from the first minutes while the rest is transcribed', 'Strömma resultat: svara på de första minuterna medan resten transkriberas'],
  'limits.todo.4': ['支持只分析选定的时间区间；上传时先识别纯音乐并提示', 'Analyse a chosen time range only; detect music-only videos on upload', 'Analysera bara ett valt tidsintervall; upptäck ren musik vid uppladdning'],
  'limits.todo.5': ['换用 GPU 版文字识别（如 PaddleOCR），长视频也能多取截图', 'GPU OCR (e.g. PaddleOCR) so long videos can keep more frames', 'GPU-baserad OCR (t.ex. PaddleOCR) så att långa videor kan behålla fler bilder'],
  'nav.theme': ['切换深色 / 浅色', 'Toggle dark / light', 'Växla mörkt / ljust'],
  ...
  'url.blocked': ['该网站拒绝了服务器的下载请求，请先把视频保存到本地，再用上传功能导入', 'The site refused the download from our server. Save the video on your device and upload the file instead.', 'Sajten nekade nedladdningen från vår server. Spara videon på din enhet och ladda upp filen i stället.'],
  'url.unsupported': ['这个链接里没有找到可下载的视频，请换成视频文件的直链（如 .mp4）', 'No downloadable video was found at this link. Try a direct link to a video file (e.g. .mp4).', 'Ingen nedladdningsbar video hittades på länken. Prova en direktlänk till en videofil (t.ex. .mp4).'],
  'url.tooLarge': ['视频超过 2 GB，无法通过链接导入', 'The video is larger than 2 GB and cannot be imported from a link.', 'Videon är större än 2 GB och kan inte importeras via länk.'],
  'url.timeout': ['连接视频网站超时，请稍后重试或改为上传文件', 'The video site did not respond in time. Try again later or upload the file.', 'Videosajten svarade inte i tid. Försök igen senare eller ladda upp filen.'],
  'msg.urlBackground': ['连接中断了，但服务器仍在后台导入，稍后列表会自动刷新', 'The connection dropped, but the server keeps importing; the list will refresh shortly', 'Anslutningen bröts, men servern fortsätter importera; listan uppdateras strax'],
