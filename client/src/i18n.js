...
const ROWS = {
  ...
  'limits.hint': ['演示服务器为 2 核 2 GB，以下为线上实测', 'Demo server: 2 cores, 2 GB. Figures measured live', 'Demoservern har 2 kärnor och 2 GB. Siffrorna är uppmätta'],
  'limits.good': ['效果好', 'Works well', 'Fungerar bra'],
  'limits.good.1': ['5 分钟以内、以讲解为主的课程或讲座片段', 'Lecture or talk clips under 5 minutes with clear speech', 'Föreläsningsklipp under 5 minuter med tydligt tal'],
  ...
  'limits.slow.1': ['10 分钟左右及更长的视频：语音转写和画面文字识别就要 5 分钟以上', 'Videos of about 10 minutes or more: transcription and on-screen text recognition alone take over 5 minutes', 'Videor på cirka 10 minuter eller mer: transkribering och textigenkänning tar över 5 minuter'],
  'limits.slow.2': ['画面切换频繁、字幕或板书密集的视频（关键帧越多越慢）', 'Fast-cutting or text-heavy videos (more key frames, more OCR)', 'Videor med många klipp eller mycket text (fler nyckelbilder, mer OCR)'],
  'limits.slow.3': ['链接导入长视频：服务器在新加坡，B 站下载约 0.3–1 MB/s，1 小时视频光下载就超过 5 分钟', 'Importing long videos by link: the server is in Singapore and Bilibili delivers 0.3–1 MB/s, so a 1-hour video takes over 5 minutes just to download', 'Långa videor via länk: servern står i Singapore och Bilibili levererar 0,3–1 MB/s, så en timmes video tar över 5 minuter bara att ladda ner'],
  ...
  'limits.row1.v': ['2.7 分钟 · 口播短视频', '2.7 min · talking-head clip', '2,7 min · pratvideo'],
  'limits.row1.t': ['约 2 分钟', '≈ 2 min', '≈ 2 min'],
  'limits.row1.c': ['¥0.08', '¥0.08', '¥0,08'],
  'limits.row1.n': ['推荐的使用方式', 'The recommended use', 'Rekommenderad användning'],
  'limits.row2.v': ['同一视频 · 换一个问题', 'Same video · new question', 'Samma video · ny fråga'],
  'limits.row2.t': ['44 秒', '44 s', '44 s'],
  'limits.row2.c': ['¥0.08', '¥0.08', '¥0,08'],
  'limits.row2.n': ['复用预处理结果', 'Preprocessing reused', 'Förbehandlingen återanvänds'],
  'limits.row3.v': ['3.2 分钟 · 纯钢琴曲', '3.2 min · solo piano', '3,2 min · solopiano'],
  'limits.row3.t': ['约 1.5 分钟', '≈ 1.5 min', '≈ 1,5 min'],
  'limits.row3.c': ['¥0.05', '¥0.05', '¥0,05'],
  'limits.row3.n': ['能跑完，但没有可分析的内容', 'Finishes, but there is nothing to analyse', 'Blir klar, men inget att analysera'],
  'limits.row4.v': ['9.9 分钟 · 数学动画', '9.9 min · maths animation', '9,9 min · matteanimation'],
  'limits.row4.t': ['> 5 分钟', '> 5 min', '> 5 min'],
  'limits.row4.c': ['¥0.06', '¥0.06', '¥0,06'],
  'limits.row4.n': ['画面切换多，预处理占了几乎全部时间', 'Many scene changes; preprocessing takes nearly all the time', 'Många klipp; förbehandlingen tar nästan all tid'],
  'limits.row5.v': ['58 分钟 · 课程录像（链接导入）', '58 min · lecture recording (by link)', '58 min · föreläsning (via länk)'],
  'limits.row5.t': ['下载 > 5 分钟', 'Download > 5 min', 'Nedladdning > 5 min'],
  'limits.row5.c': ['—', '—', '—'],
  'limits.row5.n': ['建议先剪成 5 分钟左右的片段再上传', 'Cut it into clips of about 5 minutes first', 'Klipp den i bitar på cirka 5 minuter först'],
  'limits.todo': ['后续优化方向', 'What could make it faster', 'Vad som kan göra det snabbare'],
  'limits.todo.1': ['按视频时长自适应抽帧上限，画面文字识别并行执行', 'Cap key frames by video length and run OCR in parallel', 'Begränsa nyckelbilder efter videolängd och kör OCR parallellt'],
  'limits.todo.2': ['边转写边分析：先返回前几分钟的结论', 'Stream results: answer from the first minutes while the rest is transcribed', 'Strömma resultat: svara på de första minuterna medan resten transkriberas'],
  'limits.todo.3': ['支持只分析选定的时间区间，长视频不必整段处理', 'Analyse only a chosen time range instead of the whole video', 'Analysera bara ett valt tidsintervall i stället för hela videon'],
  'limits.todo.4': ['上传时先检测有无人声和文字，纯音乐直接提示', 'Detect speech and text on upload and warn about music-only videos', 'Upptäck tal och text vid uppladdning och varna för ren musik'],
  'limits.todo.5': ['换用更强的服务器或 GPU 版文字识别（如 PaddleOCR）', 'A larger server or GPU OCR (e.g. PaddleOCR)', 'En större server eller GPU-baserad OCR (t.ex. PaddleOCR)'],
